...
async function createSession(s) {
  if (pool) {
    ...
    mem.set(s.id, {
      ...
      report_sent: false, report_draft: null
    });
  }
}
...
const ALLOWED_UPDATES = new Set(['safety_blocked', 'safety_flag', 'interview_complete', 'report_sent', 'report_draft']);
async function updateSession(id, fields) {
  const keys = Object.keys(fields).filter(k => ALLOWED_UPDATES.has(k));
  if (!keys.length) return;
  if (pool) {
    const sets = keys.map((k, i) => `${k} = $${i + 2}`).join(', ');
    const vals = keys.map(k => (k === 'report_draft' && fields[k] != null) ? JSON.stringify(fields[k]) : fields[k]);
